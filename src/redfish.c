                              ...
                              UtoolResult *result)
{
    ...
    UtoolWrapSnprintf(payload, MAX_PAYLOAD_LEN, MAX_PAYLOAD_LEN, DOWNLOAD_BMC_FILE_PAYLOAD, bmcFileUri);
...
void UtoolSftpUploadFileToBMC(UtoolRedfishServer *server, char *uploadFilePath, UtoolResult *result)
{
    ...
    if (isSshEnabledNode != NULL && cJSON_IsTrue(isSshEnabledNode)) {
        ...
        UtoolWrapSnprintf(sftpRemoteFileUrl, MAX_URL_LEN, MAX_URL_LEN, "sftp://%s:%s@%s:%d/tmp/web/%s",
                         ...
                         UtoolCurlResponse *response)
{
    ...
    for (int idx = 0;; idx++) {
        ...
        UtoolWrapSnprintf(buffer, MAX_HEADER_LEN, MAX_HEADER_LEN, "%s: %s", header->name, header->value);
    ...
    if (UtoolStringEquals(httpMethod, HTTP_PATCH) || UtoolStringEquals(httpMethod, HTTP_PUT)) {
        if (ifMatchHeader == NULL) {
            ...
            UtoolWrapSnprintf(ifMatch, MAX_HEADER_LEN, MAX_HEADER_LEN, "%s: %s", HEADER_IF_MATCH, response->etag);
                                   ...
                                   const char *httpMethod, UtoolCurlResponse *response)
{
    ...
    if (curl) {
        ...
        if (strstr(resourceURL, "%s") != NULL) {
            ...
            UtoolWrapSnprintf(_resourceURL, MAX_URL_LEN, MAX_URL_LEN, resourceURL, server->systemId);
    ...
    if (extendedInfoArray != NULL && cJSON_IsArray(extendedInfoArray)) {
        ...
        if (count > 0) {
            ...
            for (; idx < count; idx++) {
                ...
                UtoolWrapSnprintf(buffer, MAX_FAILURE_MSG_LEN, MAX_FAILURE_MSG_LEN, "[%s] %s Resolution: %s",
...
int UtoolGetRedfishServer(UtoolCommandOption *option, UtoolRedfishServer *server, char **result)
{
    ...
    UtoolWrapSnprintf(baseUrl, MAX_URL_LEN, MAX_URL_LEN, "https://%s:%d", option->host, option->port);
...
void UtoolRedfishWaitUtilTaskFinished(UtoolRedfishServer *server, cJSON *cJSONTask, UtoolResult *result)
{
    ...
    while (true) {
        ...
        if (task->taskPercentage == NULL) { /** try to print message if task has not started */
            ...
            UtoolWrapSnprintf(taskProgress, sizeof(taskProgress), sizeof(taskProgress),
